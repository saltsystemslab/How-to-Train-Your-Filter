...
filepaths = {'url': {'path': '../data/old/random_forest_malicious_url_scores.csv', 'label_name': 'type', 'pos_indicator': 'malicious', 'score_name': 'prediction_score'}, 
             'ember': {'path': '../data/old/random_forest_combined_ember_metadata.csv', 'label_name': 'label', 'pos_indicator': 1,'score_name': 'score'}, 
             'shalla': {'path': '../data/old/random_forest_shalla_combined.csv', 'label_name': 'label', 'pos_indicator': 1, 'score_name': 'score'}, 
             'caida': {'path': '../data/old/random_forest_caida.csv', 'label_name': 'Label', 'pos_indicator': 1, 'score_name': 'score'}}
...
for i, dataset in enumerate(filepaths):
    ...
    axs[i].hist([true_scores, false_scores], weights=[np.ones(len(true_scores)) / len(df), np.ones(len(false_scores)) / len(df)], bins=20, stacked=False, color=['red', 'blue'])
...
fig.legend(['Keys', 'Non-Keys'], loc="outside center right")
plt.savefig('../results/figures/combined_score_distributions_forest.pdf')
plt.clf()