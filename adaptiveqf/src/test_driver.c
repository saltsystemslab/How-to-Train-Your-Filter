...
void init_dist_results(dist_results_t *results) {
	results->exit_code = 0;
	results->insert_throughput = results->query_throughput = results->fpr = results->amortized_insert = results->amortized_query = results->amortized_adapt = results->size = 0;
}
...
test_results_t run_micro_test(size_t qbits, size_t rbits, uint64_t *insert_set, size_t insert_set_len, uint64_t *query_set, size_t query_set_len, int verbose) {
	...
	// size_t max_inserts = num_slots * target_load;
	// size_t num_inserts = insert_set_len > max_inserts ? max_inserts : insert_set_len;
	size_t num_inserts = insert_set_len;
...
// This currently only runs the negative queries to check how many false positives are found
dist_results_t run_short_test(size_t qbits, size_t rbits, uint64_t *insert_set, size_t insert_set_len, uint64_t *query_set, size_t query_set_len, uint64_t *query_labels, int verbose) {
	dist_results_t results;
	init_dist_results(&results);
	...
	size_t num_slots = 1ull << qbits;
	size_t minirun_id_bitmask = (1ull << (qbits + rbits)) - 1;
	if (verbose) fprintf(stderr, "started short test\n");
	QF qf;
	gettimeofday(&tv, NULL);
	start_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
	bool malloc_result = qf_malloc(&qf, num_slots, qbits + rbits, 0, QF_HASH_INVERTIBLE, 0);
	gettimeofday(&tv, NULL);
	end_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
	results.alloc_time = (end_time - start_time) / 1000000.0;
	if (!malloc_result) {
		results.exit_code = -1;
		return results;
	}
	...
	// store everything in microseconds for now...
	double total_insert_time = 0;
	double total_query_time = 0;
	double total_adapt_time = 0;

	qf_insert_result insert_result;
	int exit_result;
	if (verbose) fprintf(stderr, "Performing insertions...\n");
	for (i = 0; i < insert_set_len && qf.metadata->noccupied_slots < num_inserts; i++) {
		gettimeofday(&tv, NULL);
		start_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
		// Current problem: num slots is currently way higher than num_inserts, but inserts keeps segfaulting
		exit_result = qf_insert_using_ll_table(&qf, insert_set[i], 1, &insert_result, QF_NO_LOCK | QF_KEY_IS_HASH);
		gettimeofday(&tv, NULL);
		end_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
		total_insert_time += (end_time - start_time);
		if (exit_result < 0) break;
	}
	uint64_t hash = 0;
	int minirun_rank = 0;
	int fp_count = 0;
	...
	double query_time;
	double adapt_time;
	if (verbose) fprintf(stderr, "Starting queries...\n");
	for (i = 0; i < query_set_len; i++) {
		adapt_time = 0;
		if (query_labels[i] != 1) {
			num_negative++;
	...
	// find median runtimes - for now we just do the conventional way (a little slower than something like quickselect)
	// adapt_times will only have elements in the first <fp_count> indices...
	// qsort(insert_times, insert_set_len, sizeof(double), compare_double);
	// qsort(query_times, query_set_len, sizeof(double), compare_double);
	// qsort(adapt_times, fp_count, sizeof(double), compare_double);

	results.insert_throughput = insert_set_len / (total_insert_time / 1000000.0);
	results.insert_time = total_insert_time / 1000000.0;
	results.query_throughput = query_set_len / ((total_query_time + total_adapt_time)/ 1000000.0);
	results.fpr = ((double)fp_count) / ((double)fp_count + (double)num_negative);
	results.amortized_insert = (total_insert_time / 1000000.0) / insert_set_len;
	results.amortized_query = (total_query_time / 1000000.0) / query_set_len;
	results.amortized_adapt = (total_adapt_time / 1000000.0) / fp_count;
	results.size = qf.metadata->total_size_in_bytes;
...
double run_short_adversarial_test(size_t qbits, size_t rbits, uint64_t *insert_set, size_t insert_set_len, uint64_t *query_set, size_t query_set_len, uint64_t *query_labels, double prop_advers, int verbose, int* size_output, double* throughput) {
	// start by instantiating the filter and performing all insertions.
	test_results_t results;
	init_test_results(&results);

	size_t num_slots = 1ull << qbits;
	size_t minirun_id_bitmask = (1ull << (qbits + rbits)) - 1;
	if (verbose) fprintf(stderr, "started short adversarial test\n");
	QF qf;
	if (!qf_malloc(&qf, num_slots, qbits + rbits, 0, QF_HASH_INVERTIBLE, 0)) {
		return -1;
	}
	*size_output = qf.metadata->total_size_in_bytes;
	size_t num_inserts = insert_set_len;
	qf_insert_result insert_result;
	size_t i;
	...
	// first, figure out how many adversarial queries to do.
	int num_advers = (int) (query_set_len * prop_advers);
	// then, figure out how many queries we need to learn false positives from.
	int num_learn = query_set_len / 2;
	int num_negative = 0;

	// in the first phase, keep track of false positives.
	double total_query_time = 0;
	uint64_t hash = 0;
	int minirun_rank = 0;
	uint64_t first_half_fp_cnt = 0;
	uint64_t *learned_fps = malloc(sizeof(uint64_t) * query_set_len); // at most, all the queries will be false positives
	uint64_t dummy = 0;
	
	struct timeval tv;
	gettimeofday(&tv, NULL);
	uint64_t start_time = tv.tv_sec * 1000000.0 + tv.tv_usec, end_time, interval_time = start_time;
	for (i = 0; i < num_learn; i++) {
		if (query_labels[i] != 1) {
			// track the number of negative queries we run
			num_negative++;
		}
		if ((minirun_rank = qf_query_using_ll_table(&qf, query_set[i], &hash, QF_KEY_IS_HASH)) >= 0) {
			if (query_labels[i] != 1) {
				learned_fps[first_half_fp_cnt] = query_set[i];
				first_half_fp_cnt++;
				qf_adapt_using_ll_table(&qf, hash & minirun_id_bitmask, query_set[i], minirun_rank, QF_KEY_IS_HASH);
			}
		}
	...
	gettimeofday(&tv, NULL);
	end_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
	total_query_time += (end_time - start_time) / 1000000.0;

	// in second phase, every couple of queries, replace the query with the next one from the adversarial list.
	int current_advers = 0;
	uint64_t adversarial_phase_fp_cnt = 0;
	uint64_t current_query = 0;
	int num_remaining = query_set_len - num_learn;
	int space_between_advers = (int)((double)num_remaining / num_advers);
	gettimeofday(&tv, NULL);
	start_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
	int advers_count = 0;
	int current_index = -1;
	for (i = num_learn; i < query_set_len; i++) {
		// every 'num_advers' queries, we perform an adversarial query instead of one from our random list of queries.
		current_index++;
		bool adversarial = false;
		if (current_index % space_between_advers == 0) {
			current_query = learned_fps[current_advers];
			num_advers++;
			if (current_advers == first_half_fp_cnt - 1) {
				current_advers = 0;
			} else {
				current_advers++;
			}
			...
			current_query = query_set[i];
		}
		if (adversarial || (!adversarial && query_labels[i] != 1)) {
			// keep track of negative queries
			num_negative += 1;
		}
		if ((minirun_rank = qf_query_using_ll_table(&qf, current_query, &hash, QF_KEY_IS_HASH)) >= 0) {
			if (adversarial || query_labels[i] != 1) {
				// if this is a definite false positive or a query that should be negative, count the fp and adapt.
				learned_fps[first_half_fp_cnt] = query_set[i];
				adversarial_phase_fp_cnt++;
				qf_adapt_using_ll_table(&qf, hash & minirun_id_bitmask, query_set[i], minirun_rank, QF_KEY_IS_HASH);
			}
		}
	}
	fprintf(stderr, "advers: %d\n", advers_count);
	gettimeofday(&tv, NULL);
	end_time = tv.tv_sec * 1000000.0 + tv.tv_usec;
	total_query_time += (end_time - start_time) / 1000000.0;
	*throughput = (double) (query_set_len / total_query_time);

	free(learned_fps);
	// TODO - keep track of important runtime info instead of just false positives

	int64_t total_fp = first_half_fp_cnt + adversarial_phase_fp_cnt;
	double fpr = (double)total_fp / ((double)total_fp + (double)num_negative);
	fprintf(stderr, "Num negative: %d, total fp: %d, fpr: %f\n", num_negative, total_fp, fpr);
	return fpr;
}
